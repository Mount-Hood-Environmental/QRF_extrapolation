...
- Mark Roes:
    ...
    correspondence: true
institute:
- mhe_salmon: Mount Hood Environmental, 803 Monroe St, Suite 106, Salmon, Idaho, 83467, USA
...
# Pull in revised spatial domains being used in the Upper Salmon River update
US_crs = st_crs(4326) # WGS84

US_wbd = st_read(here("data/spatial_d/Upper_Salmon_WBD.shp"), quiet = TRUE) %>%
  st_transform(US_crs)

# Pulled from NF assessment. Species spatial domains mod'ed already
NF_sp_domain_sf = st_read(here("data/spatial_d/NF_sp_Domain.gpkg"), quiet = TRUE) %>%
  st_transform(US_crs)
...
# Tulley has been working with regional bio's and defined spatial extents.
US_sp_domain_sf = st_read(here("data/spatial_d/US_sp_Domain.gpkg"), quiet = TRUE) %>%
  st_transform(US_crs) 

US_sp_domain_df = US_sp_domain_sf %>%
  st_drop_geometry() %>%
  as_tibble()
...
# Merge the NF and US modified spatial domains
all_sp_domain_df = NF_sp_domain_df %>%
   mutate(chnk_NF = chnk,
          sthd_NF = sthd) %>%
   select(c("chnk_NF", "sthd_NF","UniqueID")) %>%
   right_join(., US_sp_domain_df)

all_sp_domain_df = all_sp_domain_df %>%
  mutate(sthd_revised = ifelse(is.na(sthd_NF), sthd_revised, sthd_NF),
         chnk_revised = ifelse(is.na(chnk_NF), chnk_revised, chnk_NF)) %>%
...
# redds
US_redds_sf_old_extrap = US_redds_sf_old_extrap %>%
  rename(chnk_strmnt = chnk,
         sthd_strmnt = sthd) %>%
  left_join(., all_sp_domain_df %>%
              select(c("chnk", "sthd","UniqueID"))) 
...
# summer juv
US_sum_sf_old_extrap = US_sum_sf_old_extrap %>%
  rename(chnk_strmnt = chnk,
         sthd_strmnt = sthd) %>%
  left_join(., all_sp_domain_df %>%
              select(c("chnk", "sthd","UniqueID"))) 
...
# winter juv
US_win_sf_old_extrap = US_win_sf_old_extrap  %>%
  rename(chnk_strmnt = chnk,
         sthd_strmnt = sthd) %>%
  left_join(., all_sp_domain_df %>%
              select(c("chnk", "sthd","UniqueID"))) 
...
1.  What was the strength between the covariate and the response variable based on the maximal information coefficient (MIC) value?
...
```{r qrf-relimp-summer, fig.cap = "Relative importance plots for covariates included in the revised juvenile summer QRF models", fig.width=5, fig.height=5}

load(here('output/figures/juv_summer_No_elev_figs.rda'))
rel_imp_p
```

```{r qrf-relimp-winter, fig.cap = "Relative importance plots for covariates included in the revised juvenile winter QRF models", fig.width=5, fig.height=5}

load(here('output/figures/juv_winter_No_elev_figs.rda'))
rel_imp_p

```

```{r qrf-relimp-redds, fig.cap = "Relative importance plots for covariates included in the revised QRF redds models", fig.width=5, fig.height=5}

load(here('output/figures/redds_No_elev_figs.rda'))
rel_imp_p
...
QRF_new_hab_cov_tbl %>%
  select(-Covariate) %>%
  mutate(Name = ifelse(Name == "Thalweg Exit Depth Avg", "Thalweg Exit Depth", Name))%>%
  kable(booktabs = T,
        align = "ccccccccl",
...
QRF_new_hab_cov_tbl %>%
  select(-Covariate) %>%
  mutate(Name = ifelse(Name == "Thalweg Exit Depth Avg", "Thalweg Exit Depth", Name)) %>%
  kable(align = "ccccccccl", caption = "Habitat covariates and their descriptions for three revised life stage QRF capacity models. Numbers indicate where each metric ranked in relative importance for each species. Dashes indicate the metric was not used for a given model.") %>%
  kable_styling(position = "center")
...
RF extrapolation model covariates were selected from the list of GAAs and evaluated for inclusion by examining relative importance plots (Figure \@ref(fig:rf-relimp-summer), Figure \@ref(fig:rf-relimp-winter), and Figure \@ref(fig:rf-relimp-redds) ), partial dependence plots (Section \@ref(revised-rf-extrapolation-model---partial-dependence-plots) ), and correlations between covariates. We used the previous extrapolation model as a starting point for covariate selection. CONTINUE DISCUSSION
...
load(here('output/figures/RF_extrap_juv_summer_No_elev.rda'))
ggarrange(chk_ri,
          stl_ri,
          common.legend = TRUE)
...
load(here('output/figures/RF_extrap_juv_winter_No_elev.rda'))
ggarrange(chk_ri,
          stl_ri,
          common.legend = TRUE)
```
...
load(here('output/figures/RF_extrap_redds_No_elev.rda'))
ggarrange(chk_ri,
          stl_ri,
          common.legend = TRUE)
```
...
chnk_sum_map = US_sum_sf_new_extrap %>%
  ...
  labs(title = "Chinook, summer juveniles",
       color = expression(`Capacity ` (fish/m^2)))
...
chnk_redd_map = US_redds_sf_new_extrap %>%
  ...
  labs(title = "Chinook, redds",
       color = expression(`Capacity (redds/km)`))
...
chnk_sum_comp_map = US_sum_sf_comp_extrap %>%
  ...
  theme(legend.position = "right")+#,
      #  axis.text.x = element_blank(),
       ...
       # axis.ticks = element_blank()) +
  labs(title = "Chinook, summer juveniles",
       color = expression(Delta * ` Capacity ` (fish/m^2)))
...
chnk_redd_comp_map = US_redds_sf_comp_extrap %>%
  ...
  theme(legend.position = "right")+#,
       ...
       # axis.ticks = element_blank()) +
  labs(title = "Chinook, redds",
       color = expression(Delta * ` Capacity (redds/km)`))
...
Habitat carrying capacity was estimated with the revised QRF and RF extrapolation models for Chinook salmon and steelhead during juvenile summer, juvenile winter, and redd life stages for eight watersheds in the Upper Salmon River Basin. Spatial domains for species were originally defined by Streamnet (<https://www.streamnet.org/home/data-maps/gis-data-sets/>) and spatial extents were revised based on expert knowledge from regional biologists.

## Chinook

```{r chnk-maps-combined, fig.cap = "Extrapolations of habitat capacity for Chinook salmon, by life-stage, for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=7, fig.height=9, eval = F}
...
```{r chnk-maps-sum1, fig.cap = "Extrapolations of habitat capacity for Chinook salmon juvenile summer rearing for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=5, fig.height=5,}
chnk_sum_map

```


```{r chnk-maps-win1, fig.cap = "Extrapolations of habitat capacity for Chinook salmon juvenile winter rearing for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=5, fig.height=5}
chnk_win_map

```
```{r chnk-maps-redd1, fig.cap = "Extrapolations of habitat capacity for Chinook salmon spawning (redds) for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=5, fig.height=5}
chnk_redd_map
...
```{r sthd-maps, fig.cap = "Extrapolations of habitat capacity for steelhead, by life-stage, for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=7, fig.height=9, eval = F}
...
```{r sthd-maps-sum1, fig.cap = "Extrapolations of habitat capacity for steelhead juvenile summer rearing for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=5, fig.height=5}
sthd_sum_map

```

```{r sthd-maps-win1, fig.cap = "Extrapolations of habitat capacity for steelhead juvenile winter rearing for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=5, fig.height=5}
sthd_win_map

```

```{r sthd-maps-redd1, fig.cap = "Extrapolations of habitat capacity for steelhead spawning (redds) for the eight watersheds within the Upper Salmon River Basin using the revised models.", fig.width=5, fig.height=5}
sthd_redd_map
...
Comparisons of watershed capacity estimates between the previous and revised QRF and RF extrapolation models reveal modest differences in most cases, with the exception of Chinook parr summer capacities in several watersheds. More discussion here...

## Chinook

```{r chnk-maps-comp, fig.cap = "Change in predicted Chinook salmon habitat capacity estimates from the original model and extrapolation, by life-stage, for the eight watersheds within the Upper Salmon River Basin.", fig.width=7, fig.height=9, eval = F}
...
```{r chnk-maps-comp-sum1, fig.cap = "Change in predicted juvenile Chinook salmon summer rearing habitat capacity estimates from the original model and extrapolation for  the eight watersheds within the Upper Salmon River Basin.", fig.width=5, fig.height=5}
chnk_sum_comp_map

```

```{r chnk-maps-comp-win1, fig.cap = "Change in predicted juvenile Chinook salmon winter rearing habitat capacity estimates from the original model and extrapolation for the eight watersheds within the Upper Salmon River Basin.", fig.width=5, fig.height=5}
chnk_win_comp_map

```

```{r chnk-maps-comp-redd1, fig.cap = "Change in predicted Chinook salmon spawning (redd) habitat capacity estimates from the original model and extrapolation for the eight watersheds within the Upper Salmon River Basin.", fig.width=5, fig.height=5}
chnk_redd_comp_map
...
```{r sthd-maps-comp, fig.cap = "Change in steelhead habitat capacity estimates from the original model and extrapolation, by life-stage, for the eight watersheds within the Upper Salmon River Basin.", fig.width=7, fig.height=9, eval = F}
...
```{r sthd-maps-comp-sum1, fig.cap = "Change in predicted juvenile steelhead summer rearing habitat capacity estimates from the original model and extrapolation for the eight watersheds within the Upper Salmon River Basin.", fig.width=5, fig.height=5}
sthd_sum_comp_map

```

```{r sthd-maps-comp-win1, fig.cap = "Change in predicted juvenile steelhead winter rearing habitat capacity estimates from the original model and extrapolation for the eight watersheds within the Upper Salmon River Basin.", fig.width=5, fig.height=5}
sthd_win_comp_map

```

```{r sthd-maps-comp-redd1, fig.cap = "Change in predicted steelhead spawning (redd) habitat capacity estimates from the original model and extrapolation for the eight watersheds within the Upper Salmon River Basin.", fig.width=5, fig.height=5}
sthd_redd_comp_map
...
To support the covariate selection process for the QRF capacity and RF extrapolation models,partial dependence plots were generated to illustrate the predicted effect of covariates on fish density and capacity. Partial dependence plots function similarly to traditional covariate effects plots where predictions on the response are made by altering the value of the covariate of interest while all others are fixed at mean values. Because random forest models do not place any constraints on the possible mathematical relationships between predictor and response variables, effects curves have been visualized using smoothing methods (LOESS) and may not reflect actual model behavior across the range of covariate values.

### Revised QRF habitat capacity model

```{r qrf-pdp-summer, fig.cap = "Partial dependence plots for covariates included in the revised juvenile summer QRF models",fig.width=7, fig.height=8}
...
```{r qrf-pdp-winter, fig.cap = "Partial dependence plots for covariates included in the revised juvenile winter QRF models", fig.width=7, fig.height=8}
...
```{r qrf-pdp-redds, fig.cap = "Partial dependence plots for covariates included in the revised QRF redds models", fig.width=7, fig.height=8}
...
```{r rf-pdp-summer, fig.cap = "Partial dependence plots for covariates included in the revised juvenile summer RF extrapolation models", fig.width=7, fig.height=8}
...
```{r rf-pdp-winter, fig.cap = "Partial dependence plots for covariates included in the revised juvenile winter RF extrapolation models", fig.width=7, fig.height=8}
...
```{r rf-pdp-redds, fig.cap = "Partial dependence plots for covariates included in the revised redds RF extrapolation models", fig.width=7, fig.height=8}
...
``` {r spawn-rec-validation, eval = T}
load(here("data/spawn_rec_params.rda"))
